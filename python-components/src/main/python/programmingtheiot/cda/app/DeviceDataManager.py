...
class DeviceDataManager(IDataMessageListener):
	"""
	The `DeviceDataManager` class manages system performance, sensing, and actuation capabilities by handling incoming actuator and sensor messages, as well as coordinating communication between various components. It utilizes configuration settings to enable or disable functionalities and processes actuator commands based on sensor data analysis, ensuring effective device management.
	...
	def _handleUpstreamTransmission(self, resource = None, msg: str = None):
		...
		logging.debug(f"Upstream transmission invoked for resource: {resource}, message: {msg}")
